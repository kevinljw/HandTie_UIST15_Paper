...
% Third, finger-based gestural interfaces are with high accuracy rates. A nearby location to the fingers is the back of hand abound with tendons involving finger flexions. Our measurement accuracy rate is 95.8\% in a 10-fold cross validation.
...
% Our prototype, \getTitleName, uses a total of 19 strain sensors in a 2-row configuration to measure the physical changes on the back of hands. [add implementation summary]. To better understand the areas that provide better [TODO: describe the studies in more details and present the results].

% Our prototype, \getTitleName, uses a total of 19 strain sensors in a 2-row configuration to measure the physical changes on the back of hands at a sampling rate of 526 Hz. A customized Arduino Nano shield circuit board is built for sensor signal conditioning. To better understand the areas that provide better gestural recognition accuracy, our prototype is affixed to 8 specified rows evenly spaced from the metacarpophalangeal joints (MCP, the knuckles between the hand and fingers) to the head of ulna (tip of the wrist) on the participants' back of hand for data collection. The optimal sensing location on the back of hand is found on the second row and third row from the MCP joints. 
...
% Some of data glove-like systems use inertial-based sensors such as accelerometer and gyro sensors to determine hand position and orientation. Several products are released recent years for abundant applications with complex sensors.
...
% In principle, there are 37 classes in the dataset including 11 digits (e.g. 0-10) and 26 alphabet letters.
% However, without considering the hand orientation and movement, there are gestures that are notoriously difficult to classify between the set of letters and digits due to their similarities.
...
% \begin{figure}
%  \begin{center}
%   \includegraphics[width=1\columnwidth]{figures/Num8GestureV1.pdf}
% %   \caption{
...
% \begin{figure*}
%  \begin{center}
%   \includegraphics[width=2\columnwidth]{figures/SVisual_v3.pdf}
%   \caption{
%     (A) A gesture of ``8'' from ASL is performed.
%     (B) A heat map is illustrated to show the sensor values of each strain gauge resulting from the gesture of ``8'' performed by a user. 
...
%   \caption{
%     Heat maps indicate the highest tension and highest compression sensor value as red and blue, respectively.
%     (A) A gesture of ``8'' from ASL is performed.
...
%     % Our sensing location starts with ``row-0'' at the metacarpophalangeal joints and ends with ``row-7'' at the head of ulna.
%     (C) Each entry is an average heat map of 10 trials of gesture ``8'' performed by a single user. The color distributions of each heat map differ quite much from person to person.
%     (D) 10 heat maps display all 10 trials of gesture ``8'' carried out by a participant. The color distributions remain fairly consistent throughout the trials.
%   }
...
\begin{figure*}
 \begin{center}
  ...
  \caption{
    % The result of the average of 10-fold cross validations of all 10 participants. The value of each bar is a average accuracy rate of each of the 10 folds undergoing a 10-fold cross validation at a location on the back of hand. The average accuracy rates of all 15 subsets including 8 single-row configuration and 7 two-adjacent row configuration of sensing locations are used for the determination of the optimal sensing location on the back of hand.
    % The value of each bar is an average accuracy rate of each fold validated by the training model constructed by other 9 folds at a location on the back of hand.
